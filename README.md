...
An implementation of unboxed data structures for OCaml by Robert Atkey
<bob.atkey@gmail.com>.
...
The `unboxed` library implements functors that generate new abstract
...
library uses unsafe operations from the `Obj` module in its
...
The functor `Unboxed.PolymorphicVariantArray.Make` is similar to the
...
`Unboxed.PolymorphicVariantArray.Make` functor for generating
